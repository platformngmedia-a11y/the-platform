...
export function CategoryFilterSidebar({ articles, categorySlug }: Props) {
  ...
  const filteredArticles = useMemo(() => {
    ...
    if (sortBy === 'newest') {
      ...
      // For now, use word count as proxy for "popular"/"in-depth"
  ...
  return (
    <>
      ...
      <button
        ...
        className="lg:hidden flex items-center gap-2 px-4 py-2.5 bg-navy text-white rounded-lg font-semibold hover:bg-navy-dark transition-colors mb-6 w-full justify-center"
      >
        <Filter size={16} />
        Filters
      ...
      {/* Sidebar */}
      <div
        className={`fixed lg:relative inset-y-0 left-0 w-64 bg-white border-r border-line z-50 transform transition-transform lg:transform-none lg:translate-x-0 ${
          isOpen ? 'translate-x-0' : '-translate-x-full'
        } lg:bg-transparent lg:border-none p-6 overflow-y-auto`}
      >
        {/* Close button mobile */}
        <button
          onClick={() => setIsOpen(false)}
                ...
                return (
                  <label key={type} className="flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      onChange={() => setContentType(contentType === type ? null : type)}
                      className="w-4 h-4 cursor-pointer"
                    />
              ...
              return (
                <label key={level.value} className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    onChange={() => setTrustLevel(trustLevel === level.value ? null : level.value)}
                    className="w-4 h-4 cursor-pointer"
                  />
